    ...
    for key in list(contours):
        ...
        if key == 1:
            for idx, val in enumerate(contours.get(key)):
                if idx == 0:
                    pst_2 = contours.get(2)[0]
                    mssv = draw_rectangle(image, val, pst_2, 180, 110)
                    not_block = True
                    continue
                if idx == 1:
                    pst_2 = contours.get(2)[1]
                    ma_de = draw_rectangle(image, val, pst_2, 180, 80)
                    not_block = True
                    continue
        ...
        for idx, val in enumerate(contours.get(key)):
            # if idx == 0:
            pst_2 = contours.get(key)[0]
            block = draw_rectangle(image, val, pst_2, 180, 80)
...
def find_circle(block, is_vertical=False):
...
def grade_block(images, circles, batch, type=None):
    answer = dict()
    for image in images:
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        mssv_thresh = cv2.threshold(gray, 150, 255, cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)[1]
        cv2.imwrite('mssv_thresh.jpg', mssv_thresh)
        ...
        for (q, i) in enumerate(np.arange(0, len(circles), batch)):
            ...
            for (inner_idx, circle) in enumerate(ctns):
                ...
                rectX = (x - r)
                rectY = (y - r)
                crop_img = mssv_thresh[rectY:(rectY + 2 * r), rectX:(rectX + 2 * r)]
...
def grade_multiple_choice_quiz(mssv, ma_de, blocks):
    # ma so sinh vien
    mssv_circles, batch = find_circle(mssv, True)
    grade_block([mssv], mssv_circles, batch)

    # ma de
    ma_de_circles, batch = find_circle(ma_de, True)
    grade_block([ma_de], ma_de_circles, batch)
    ...
    for idx, sub_blocks in enumerate(blocks, start=1):
        for idx_sub, sub_block in enumerate(sub_blocks):
            ...
            # cv2.imshow(f'{i}', test)
            block_circles, batch = find_circle(sub_block)
            grade_block([sub_block], block_circles, batch)
            i += 1


def main():
    image = cv2.imread("img/IMG_6103.JPG")
    image = image_resize(image, 800, 800)
    # image = cv2.rotate(image, cv2.ROTATE_90_COUNTERCLOCKWISE)
    multile_choice_quiz = detect_multiple_choice_quiz(image)
    core_component = detect_core_components(multile_choice_quiz)
    small_components_contour = detect_small_components(core_component)
    mssv, ma_de, blocks = draw_block(small_components_contour, core_component)
    cv2.imwrite('mssv_new.jpg', mssv)

    # cv2.imshow('mssv', mssv)
    # cv2.imshow('ma_de', ma_de)
    grade_multiple_choice_quiz(mssv, ma_de, blocks)
    cv2.waitKey(0)
